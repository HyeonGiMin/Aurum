...
| **실행 즉시 전체 fetch** | Options 의 *실행 즉시 전체 fetch* — **Golden 의 원래 기본 동작**입니다(점진 fetch 는 Golden 8 에서 옵션으로 추가된 것). 켜면 결과를 끝까지 가져와 **로드된 행 수 = 전체**가 되고 **스크롤바가 정확**해집니다. 기본은 꺼짐 — 대용량 결과에서 느리므로 *최대 로드 행수* 와 함께 쓰세요 |
