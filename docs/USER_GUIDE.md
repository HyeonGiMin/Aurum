...
- SSH 비밀번호와 키 passphrase 는 DB 비밀번호와 **똑같이 AES-256 으로 암호화**되어
  저장되고, Login List 항목에 함께 딸려 옵니다.
  - **비밀번호·passphrase 저장** 체크를 끄면 **디스크에 아예 남기지 않고**
    접속할 때마다 물어봅니다(pgAdmin 의 *Prompt for password?* 대응). 저장을 끈 항목은
    로그온 창에 `via ops@jump (비밀번호 필요)` 로 표시되고, Login 을 누르면 비밀번호
    칸으로 바로 들어갑니다. 껐다는 것은 "잘 숨겨라"가 아니라 "두지 마라"이므로
    **암호문으로도 남기지 않습니다.**

#### 호스트 키 확인

터널을 세우기 전에 **SSH 서버의 신원을 확인**합니다 (pgAdmin·OpenSSH 와 같은 규칙).
이게 없으면 중간에 낀 서버가 bastion 인 척하고 DB 비밀번호를 그대로 받아갈 수 있습니다.

- **처음 붙는 호스트** — 키 지문(`SHA256:…`, `ssh` 명령과 같은 표기)을 보여주고
  받아들일지 묻습니다. **기본은 거부**입니다. 서버 관리자에게 받은 값이나
  서버에서 `ssh-keygen -lf /etc/ssh/ssh_host_ed25519_key.pub` 로 얻은 값과 대조하세요.
- **키가 바뀐 호스트** — 알던 지문과 받은 지문을 나란히 보여주는 **경고**가 뜹니다.
  서버를 다시 깔았다면 정상이지만, 그렇지 않다면 가로채기입니다. 역시 기본은 거부입니다.
- **이미 아는 호스트** — 아무것도 묻지 않습니다.
- 참조하는 파일:
  - `~/.ssh/known_hosts` — **읽기만** 합니다. 터미널에서 이미 그 bastion 에 붙어 본
    적이 있으면 처음부터 아무것도 묻지 않습니다. 해시된 항목(`ssh-keyscan -H`)도 읽습니다.
  - `~/.prismone-studio/known_hosts` — 이 앱에서 승인한 키가 쌓이는 곳. **쓰기는 여기만**
    합니다(다른 도구가 쓰는 파일을 GUI 가 말없이 고치지 않습니다). OpenSSH 와 같은
    형식이라 열어 보고 줄을 지울 수 있습니다.
  - `@revoked` 로 표시된 키는 다른 줄이 허용하고 있어도 **무조건 거부**합니다.
...
- **MongoDB 도 됩니다** (2026-09-08). 다만 규칙이 다릅니다:
  - 원본 문서를 알 수 있는 **순수 `find` 결과만** 편집합니다 (projection·aggregate 는 거부).
    행은 의사 컬럼이 아니라 **`_id`** 로 찾고, `_id` 자체는 고칠 수 없습니다.
  - 셀 값은 **원래 BSON 타입을 지켜** 저장합니다. 숫자 필드에 글자를 넣으면 저장 전에 막습니다
    (Mongo 는 스키마가 없어 그냥 두면 타입이 조용히 바뀝니다).
  - 배열·중첩 문서 칸은 그리드에서 고칠 수 없습니다 — **Edit Document (Ctrl+Shift+D)** 를 쓰세요.
  - **트랜잭션이 없습니다.** ✓ Post 하면 문서 하나씩 즉시 반영되고 Commit/Rollback 으로
    되돌릴 수 없습니다. 중간에 실패하면 그 앞까지는 이미 반영된 채로 멈추고, 몇 건이
    들어갔는지 알려 줍니다.
...
- 팝업이 뜬 뒤 계속 타이핑하면 필터링, Enter/Tab 으로 삽입. 각 줄은 종류 배지
  (**T**able · **V**iew · **S**chema · **C**olumn · **K**eyword) + 이름 + 오른쪽 설명이고,
  배지 색은 라이트/다크에 맞춰 바뀝니다.
- **언제 뜰지는 Options 에서 고릅니다** (`자동완성 팝업`):
  - **자동** (기본) — 위에 적은 대로 `.` 과 FROM/JOIN 뒤에서 알아서 뜨고 Ctrl+Space 도 됩니다.
  - **Ctrl+Space 로만** — Golden 방식. 타자 중에는 뜨지 않고 부를 때만 뜹니다.
  - **쓰지 않음** — Ctrl+Space 도 막습니다.
...
- 왼쪽에서 **테이블**을 고르고 **쿼리에 추가**(또는 더블클릭) 합니다. 첫 테이블은 `from`,
  그 다음부터는 **join** 으로 붙습니다. 별칭은 이름 첫 글자로 자동 배정됩니다 (`s`, `p`, `s2`…).
- **조인**: 줄마다 `join / left join / right join / full join` 을 고를 수 있고,
  **ON 조건은 카탈로그의 FK 에서 찾아 채워 줍니다.** 못 찾으면 상태줄이 알려 주고
  `s.patient_key = p.patient_key` 처럼 직접 적으면 됩니다 (`and` 로 여러 쌍 가능).
  ✕ 로 그 테이블을 뺍니다.
- 컬럼은 테이블별로 묶여 나옵니다. 체크한 컬럼만 SELECT 되고, 아무것도 체크하지 않으면 `*` 입니다.
- **distinct** 체크와 **+ 집계**(`count · sum · avg · min · max`, 별칭 지정 가능)를 쓸 수 있습니다.
  집계를 하나라도 넣으면 **체크한 컬럼이 자동으로 GROUP BY** 가 됩니다. `count` 는 컬럼을
  비워 두면 `count(*)` 입니다.
...
- Order by · Limit 를 지정할 수 있고, 아래 **미리보기**에 완성된 SQL 이 실시간으로 보입니다.
  Limit 는 접속한 DB 에 맞춰 나갑니다 (Oracle 은 `fetch first N rows only`).
