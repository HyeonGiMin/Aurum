...
6. ~~**타이포 스케일 통일**~~ — **검토 후 보류 (2026-08-05).** 실측하니 이미
   사실상 스케일로 수렴해 있다: 본문 13(38곳) · 그리드 12(47곳) · 메뉴/버튼
   12.5(13곳) · 캡션 11/11.5(20곳), 이탈은 7곳뿐. 12.5 는 Golden 크롬의
   일부라 강제 통일은 정체성 대비 이득이 없고 스크린샷 diff 만 키운다.
   새 UI 는 이 스케일을 따른다 (본문 13 · 그리드 12 · 캡션 11).
7. ~~**툴바 아이콘 정리**~~ ✅ 점검 완료 (2026-08-05) — 아이콘은 전부 같은
   방식(StreamGeometry 수제 도형)이고 hover/pressed/disabled/구분선은 테마
   리소스로 이미 통일돼 있다 (다크 모드 작업에서 정리됨).
